...
def test_config(config):
    ...
    try:
        ...
        out = convtranspose2d(inp)
        del convtranspose2d, inp, out
        return True
    except Exception as e:
        print(e)
    
    del inp
    # del convtranspose2d, inp
    return False
...
def index_filter(args, index):
    ...
    image_size = config[1] * config[2] * config[2] * config[3]

    out_image_size = (config[2]-1)*config[6] - 2*config[7] + (config[5]-1) + 1
    ...
    #       vice versa).
    # return conv_size <= 35000000 and image_size <= 1024*1024*1024 and out_image_size > 0 and test_config(config)
    return conv_size <= 35000000 and image_size <= 512*1024*1024 and out_image_size > 0 
...
def config_to_profiler_args(config):
    ...
    convtranspose2d = to_cuda(torch.nn.ConvTranspose2d(
...
def main():
    measurer = Measurer(
        ...
        # we're still using the same set of configs as conv2d
